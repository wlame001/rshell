...
int Interface::call_test(int i){
    Test* t = new Test(cmd.at(i));// construct a test object from cmd.at(i)
    bool runVal = t->run();
    if(runVal == 0) // if the test failed, was_executed = 0
        return 0;
    return 1;    
}
...
bool Interface::run(){
    ...
    for (unsigned i = 0; i < cmd.size(); i++){// iterate for each command
        if(strcmp(cmd.at(i).args.at(0), "test" ) == 0 || // if the test command is used
        (strcmp(cmd.at(i).args.at(0), "[" ) == 0)){
            was_executed = call_test(was_executed);
        }
        else{
            callFork(child_pid);
